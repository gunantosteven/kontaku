...
class TopTotalContactsController extends Controller {
	...
	*/
	...
	public function index()
	{
		if(Input::has('search'))
		{
			...
			->select('id', 'fullname', 'url', 'limitcontacts', 'membertype')
		...
		foreach ($users as $user)
		{
			...
			$array[$count++] = array("fullname" => $user->fullname, "url" => $user->url, "totalcontacts" => $totalcontacts, "membertype" => $user->membertype, "limitcontacts" => $user->limitcontacts);
		}

		$array = array_reverse(array_sort($array, function($value)
		{
		    return $value['totalcontacts'];
		}));
		...
		return view('/admin/toptotalcontacts', ['array' => $paginatedSearchResults]);
	}
